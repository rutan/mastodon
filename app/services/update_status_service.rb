...
class UpdateStatusService < BaseService
  ...
  def validate_media!
    ...
    raise Mastodon::ValidationError, I18n.t('media_attachments.validations.too_many') if @options[:media_ids].size > MediaAttachment::MAX_UPLOAD_SIZE || @options[:poll].present?

    media_attachments = @status.account.media_attachments.where(status_id: [nil, @status.id]).where(scheduled_status_id: nil).where(id: @options[:media_ids].take(MediaAttachment::MAX_UPLOAD_SIZE).map(&:to_i)).to_a
